...
export async function getRaces(): Promise<RaceRow[]> {
  try {
    const response = await axios.get<any[]>('http://localhost:3000/races')
    return response.data.map((race) => {
      ...
      return {
        ...
        startTime: startDateTime
          ...
          : '-',
        course: race.course,
        raceClassId: race.raceClass?.id ?? 0,
        seriesId: race.series?.id ?? 0,
...
export async function addRace(payload: RacePayload): Promise<void> {
  try {
    const response = await axios.post('http://localhost:3000/race', payload)
